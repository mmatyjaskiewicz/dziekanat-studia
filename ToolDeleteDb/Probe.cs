...
Console.WriteLine($"status={(int)r2.StatusCode} body={s.Substring(0, Math.Min(500, s.Length))}");

var r3 = await c.PostAsJsonAsync("/api/students/00000000-0000-0000-0000-000000000000/change-status", new { status = 2, reason = "test" });
var s3 = await r3.Content.ReadAsStringAsync();
Console.WriteLine($"change-status status={(int)r3.StatusCode} body={s3.Substring(0, Math.Min(500, s3.Length))}");
